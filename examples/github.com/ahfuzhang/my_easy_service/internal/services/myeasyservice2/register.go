...
func Register(framework netframework.Framework) {
	...
	engine.POST("/cheaprpc.ahfuzhang.my_easy_service.MyEasyService2.GetEchoInfo", func(ctx *gin.Context) {
	...
	engine.POST("/cheaprpc.ahfuzhang.my_easy_service.MyEasyService2.Save", func(ctx *gin.Context) {
		body, err := ctx.GetRawData()
		if err != nil {
			ctx.JSON(http.StatusBadRequest, map[string]interface{}{
				"code": 400,
